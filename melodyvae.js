...
function getNoteIndexAndTimeshift(note, tempo) {
  ...
  const index = Math.max(0, Math.floor((note.time + half_unit) / unit)); // Quantize to the CENTER of the grid line
  const timeshift = (note.time - unit * index) / half_unit; // Normalize to -1 to +1 range
...
function processPianoroll(midiFile, augmentation) {
  ...
  midiFile.tracks.forEach((track) => {
    if (track.channel != 9 && track.channel != 10) {
      // ignore drum tracks
      //notes are an array
      ...
      notes.forEach((note) => {
        ...
        if (pitch < NUM_MIDI_CLASSES) {
          ...
          while (Math.floor(index / LOOP_DURATION) >= onsets.length) {
            ...
            velocities.push(
              utils.create2DArray(NUM_MIDI_CLASSES, LOOP_DURATION)
            );
            durations.push(
              utils.create2DArray(NUM_MIDI_CLASSES, LOOP_DURATION)
            );
    ...
    for (let t = 0; t < original_count; t++) {
      ...
      for (let diff of transpositions) {
        if (maxv + diff <= MAX_MIDI_NOTE && minv + diff >= MIN_MIDI_NOTE) {
          ...
          let newvelocity = utils.create2DArray(
            NUM_MIDI_CLASSES,
            LOOP_DURATION
          );
          let newduration = utils.create2DArray(
            NUM_MIDI_CLASSES,
            LOOP_DURATION
          );
          let newtimeshift = utils.create2DArray(
            NUM_MIDI_CLASSES,
            LOOP_DURATION
          );
  ...
  console.assert(
    onsets.length == velocities.length &&
      velocities.length == durations.length &&
      durations.length == timeshifts.length,
  ...
  for (var i = 0; i < onsets.length; i++) {
    if (getNumOfOnsets(onsets[i]) > MIN_ONSETS_THRESHOLD) {
      train_data_onsets.push(
        tf.tensor2d(onsets[i], [NUM_MIDI_CLASSES, LOOP_DURATION])
      );
      train_data_velocities.push(
        tf.tensor2d(velocities[i], [NUM_MIDI_CLASSES, LOOP_DURATION])
      );
      train_data_durations.push(
        tf.tensor2d(durations[i], [NUM_MIDI_CLASSES, LOOP_DURATION])
      );
      train_data_timeshifts.push(
        tf.tensor2d(timeshifts[i], [NUM_MIDI_CLASSES, LOOP_DURATION])
      );
    }
  }
}
...
async function generatePattern(
  ...
  if (vae.isReadyToGenerate()) {
    ...
    isGenerating = true;
    let [onsets, velocities, durations, timeshifts] = vae.generatePattern(
      z1,
      z2,
      noise_range
    );
    ...
    for (var j = 0; j < LOOP_DURATION; j++) {
      ...
      if (bestIdx >= 0) {
        ...
        duration_sequence.push(
          Math.min(Math.floor(durations[bestIdx][j] * 64), 127)
        );
        // Apply RhythmVAE-style timeshift scaling: -1 to +1 -> 0 to 127, then apply grid_offset
        var scaledTimeshift =
          Math.floor(utils.scale(timeshifts[bestIdx][j], -1, 1, 0, 127)) *
          grid_offset;
    ...
    for (var k = 1; k < 16; k++) {
      ...
      Max.outlet(
        "velocity_output",
        k + 1,
        Array(LOOP_DURATION).fill(0).join(" ")
      );
      Max.outlet(
        "duration_output",
        k + 1,
      ...
      Max.outlet(
        "timeshift_output",
        k + 1,
        Array(LOOP_DURATION).fill(0.0).join(" ")
      );
    }
...
Max.addHandler("encode_add", (pitch, time, duration, velocity, muted) => {
  ...
  if (!muted) {
    ...
    const normalizedDuration = Math.max(
      0.1,
      Math.min(duration / durationUnit, 2.0)
    );
...
Max.addHandler("encode_done", () => {
  ...
  var inputTime = tf.tensor2d(input_timeshift, [
    NUM_MIDI_CLASSES,
    LOOP_DURATION,
  ]);
